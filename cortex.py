...
class Cortex():
    ...
    def query_headset(self):
        print('query headset --------------------------------', flush=sys.stdout)
    ...
    def connect_headset(self):
        print('connect headset --------------------------------', flush=sys.stdout)
        ...
        if self.debug:
            print('connect headset result', json.dumps(result_dic, indent=4), flush=sys.stdout)


    def request_access(self):
        print('request access --------------------------------', flush=sys.stdout)
    ...
    def authorize(self):
        print('authorize --------------------------------', flush=sys.stdout)
        authorize_request = {
            "jsonrpc": "2.0",
            "method": "authorize", 
            "params": { 
        ...
        if self.debug:
            print('auth request \n', json.dumps(authorize_request, indent=4), flush=sys.stdout)
        ...
        while True:
            ...
            if 'id' in result_dic:
                if result_dic['id'] == AUTHORIZE_ID:
                    if self.debug:
                        print('auth result \n', json.dumps(result_dic, indent=4), flush=sys.stdout)
                    self.auth = result_dic['result']['cortexToken']
                    break


    def create_session(self, auth, headset_id):
        print('create session --------------------------------', flush=sys.stdout)
        ...
        if self.debug:
            print('create session request \n', json.dumps(create_session_request, indent=4), flush=sys.stdout)
        while True:
            self.ws.send(json.dumps(create_session_request))
            result = self.ws.recv()
            result_dic = json.loads(result)
            if('result' in result_dic) and ('id' in result_dic['result']):
                break
        if self.debug:
    ...
    def close_session(self):
        print('close session --------------------------------', flush=sys.stdout)
    ...
    def get_cortex_info(self):
        print('get cortex version --------------------------------', flush=sys.stdout)
    ...
    def disconnect_headset(self):
        print('disconnect headset --------------------------------', flush=sys.stdout)
        ...
        while True:
            ...
            if self.debug:
                print('disconnect headset result', json.dumps(result_dic, indent=4), flush=sys.stdout)
    ...
    def sub_request(self, stream, emit=None):
        print('subscribe request --------------------------------', flush=sys.stdout)
        sub_request_json = {
            "jsonrpc": "2.0", 
            "method": "subscribe", 
            "params": { 
        ...
        if 'sys' in stream:
            ...
            while True:
                ...
                # print(dict_data)
                if('result' in dict_data and 'success' in dict_data['result']):
                    if('cols' in dict_data['result']['success'][0]):
                        cols = dict_data['result']['success'][0]['cols']
                        stream_name = dict_data['result']['success'][0]['streamName']
                if stream_name in dict_data:
                    clean_data = {
                        "cols": cols,
                        "stream_data": dict_data[stream_name]
                    }
                    # print(clean_data, flush=sys.stdout)
                    i = 0
                    line_data = {}
                    column_data = {}
                    while i < len(clean_data["cols"]):
                        chanel_index = clean_data["cols"][i].split('/')
                        # print(chanel_index, ":", clean_data['stream_data'][i], flush=sys.stdout)
                        chanel = str(chanel_index[0])
                        index = chanel_index[1]
                        value = clean_data['stream_data'][i]
                        # print(chanel, index, value, flush=sys.stdout)
                        if (chanel not in line_data):
                            line_data[chanel] = {index: value}
                        else:
                            line_data[chanel][index] = value
                        i += 1

                        if(chanel not in column_data):
                            column_data[chanel] = {
                                "name" : chanel,
                                "data": [value]
                            }
                        else:
                            column_data[chanel]["data"].append(value)
                    engagement_sum = 0
                    number_chanel = 0
                    engagement_average = 0
                    for line in line_data:
                        line_data[line]["engagement"] = round((line_data[line]['betaH']/(line_data[line]['alpha'] + line_data[line]['theta'])), 3)
                        engagement_sum = engagement_sum + line_data[line]["engagement"]
                        number_chanel = number_chanel + 1
                    if(number_chanel != 0):
                        engagement_average = round((engagement_sum/number_chanel), 3)
                    print("Engagement average: ", engagement_average, flush=sys.stdout)
                    clean_data["line_data"] = line_data
                    clean_data["column_data"] = column_data
                    clean_data["engagement_average"] = engagement_average
                    emit('draw_chart', clean_data)
                    # time.sleep(0.125)0
                    time.sleep(2)
    def query_profile(self):
        print('query profile --------------------------------', flush=sys.stdout)
        query_profile_json = {
            "jsonrpc": "2.0",
            "method": "queryProfile",
            "params": {
        ...
        if self.debug:
            print('query profile request \n', json.dumps(query_profile_json, indent=4), flush=sys.stdout)
            print('\n', flush=sys.stdout)
        ...
        print('extract profiles name only', flush=sys.stdout)
        print(profiles, flush=sys.stdout)
        print('\n', flush=sys.stdout)
    ...
    def setup_profile(self, profile_name, status):
        print('setup profile --------------------------------', flush=sys.stdout)
        setup_profile_json = {
            "jsonrpc": "2.0",
            "method": "setupProfile",
            "params": {
        ...
        if self.debug:
            print('setup profile json:\n', json.dumps(setup_profile_json, indent=4), flush=sys.stdout)
            print('\n', flush=sys.stdout)
    ...
    def train_request(self, detection, action, status):
        ...
        if status == 'start':
            ...
            print('\n YOU HAVE 8 SECONDS FOR TRAIN ACTION {} \n'.format(action.upper()), flush=sys.stdout)
        ...
        while True:
            ...
            print(json.dumps(result_dic, indent=4), flush=sys.stdout)
    ...
    def create_record(self,
                    record_name,
                    record_description):
        print('create record --------------------------------', flush=sys.stdout)
        ...
        while True:
            ...
            result = self.ws.recv()
            result_dic = json.loads(result)
            # print('Hey start record result \n', json.dumps(result_dic, indent=4), flush=sys.stdout)
            if ('result' in result_dic) and ('record' in result_dic['result']):
                break

        if self.debug:
            print('start record request \n', json.dumps(create_record_request, indent=4), flush=sys.stdout)
    ...
    def stop_record(self):
        print('stop record --------------------------------', flush=sys.stdout)
        stop_record_request = {
            "jsonrpc": "2.0", 
            "method": "stopRecord",
            "params": {
        ...
        if self.debug:
            print('stop request \n', json.dumps(stop_record_request, indent=4), flush=sys.stdout)
            print('stop result \n', json.dumps(result_dic, indent=4), flush=sys.stdout)
    ...
    def export_record(self, 
                    ...
                    export_version,
                    record_ids):
        print('export record --------------------------------', flush=sys.stdout)
        ...
        if self.debug:
            print('export record request \n',
                json.dumps(export_record_request, indent=4), flush=sys.stdout)
    ...
    def inject_marker_request(self, marker):
        try:
            print('inject marker --------------------------------', flush=sys.stdout)
            ...
            while True:
                ...
                result_dic = json.loads(result)
                if self.debug:
                    print('inject marker request \n', json.dumps(inject_marker_request, indent=4), flush=sys.stdout)
    ...
    def update_marker_request(self, marker):
        try:
            print('update marker --------------------------------', flush=sys.stdout)
            ...
            while True:
                ...
                result_dic = json.loads(result)
                if self.debug:
                    print('update marker request \n', json.dumps(update_marker_request, indent=4), flush=sys.stdout)
                    print('update marker result \n', json.dumps(result_dic, indent=4), flush=sys.stdout)
    ...
    def get_mental_command_action_sensitivity(self, profile_name):
        print('get mental command sensitivity ------------------', flush=sys.stdout)
        sensitivity_request = {
    ...
    def set_mental_command_action_sensitivity(self, 
                                            profile_name, 
                                            values):
        print('set mental command sensitivity ------------------', flush=sys.stdout)
        sensitivity_request = {
                                "id": SENSITIVITY_REQUEST_ID,
                                "jsonrpc": "2.0",
                                "method": "mentalCommandActionSensitivity",
    ...
    def get_mental_command_active_action(self, profile_name):
        print('get mental command active action ------------------', flush=sys.stdout)
    ...
    def get_mental_command_brain_map(self, profile_name):
        print('get mental command brain map ------------------', flush=sys.stdout)
        brain_map_request = {
    ...
    def get_mental_command_training_threshold(self, profile_name):
        print('get mental command training threshold -------------', flush=sys.stdout)
